...
@Component
@Scope("prototype")
public class ResponsiblePartnerCompany extends ElementXML {

    private static final String S1000D_TITLE = "Responsible Partner Company: ";
    ...
    public String toHtml() {
        ...
        List<String> usedAttributes = Arrays.asList("enterpriseCode");
        Element span = div.appendElement(HTML_SPAN).text(S1000D_TITLE);
        span.addClass("bold");
        StringBuilder content = new StringBuilder();
        String mfc = this.getAttribute("enterpriseCode");
        content.append(mfc);
        if (mfc != null && !mfc.isEmpty()){
            content.append(" ");
            content.append(DASH);
        }
        //add other attributes
        div.appendElement(HTML_SPAN).text(content.toString());
